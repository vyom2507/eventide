...
import { ArrowDown, ArrowUpRight, CalendarDays, Check, MapPin, Users } from 'lucide-react'
...
function validate(form: FormData): Errors {
  ...
  if (form.phone && !/^(?:\+?1[\s.-]?)?(?:\(?[2-9]\d{2}\)?[\s.-]?)\d{3}[\s.-]?\d{4}$/.test(form.phone.trim())) errors.phone = 'Please enter a valid US phone number.'
...
export default function Page() {
  ...
  const scrollToRegistration = () => document.getElementById('register')?.scrollIntoView({ behavior: 'smooth', block: 'start' })
  ...
  const resetForm = () => { setForm(emptyForm); setErrors({}); setDuplicate(false); setSubmitted(false); window.setTimeout(scrollToRegistration, 0) }
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      <header className="mx-auto flex w-full max-w-7xl items-center justify-between px-6 py-6 lg:px-10">
        ...
        <button type="button" onClick={scrollToRegistration} className="hidden items-center gap-2 text-sm font-medium text-primary transition-opacity hover:opacity-70 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-primary sm:flex">Register for free <ArrowUpRight aria-hidden="true" /></button>
      </header>
